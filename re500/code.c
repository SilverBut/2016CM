...
TASK(TaskShowLCD)
{
    ...
    lcd_init();
    static char long_flag[FLAGLEN] = FLAG;
    for ( int pos = 0 ; pos < FLAGLEN ; pos++){
    	for ( int cur = 0 ; cur < MAPLEN ; cur++ ){
    		if (charmap[cur].ch == long_flag[pos]){
    			lcd_set_custom_char(1,charmap[cur].map);
    			lcd_sendchar(1,0,pos%10);
    			goto nextchr;
    		}
    	}
    	while(1);
